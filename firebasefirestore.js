...
// Retrieve and parse product data from localStorage
...
document.addEventListener('click', async (event) => {
  if (event.target.classList.contains('proceed-payment')) {
    ...
    const clientIDListRef = collection(db, 'organizations', orgDocId, 'ClientID_List');
    const totalPrice = parseFloat(event.target.getAttribute('data-product-total-price'));
    ...
    const copyClientIDListToOrder = async () => {
      try {
        ...
        await setDoc(doc(ordersRef, orderId), {
          ...
          // tableName and additionalDetails should be defined or retrieved from the appropriate source
          tableName: "", 
          note: "", 
          subTotal: 0.0,
          confirmedBy: "",
    ...
    for (const item of cart) {
      ...
      await copyClientIDListToOrderItem(item);
    }
    await addOrder(totalValue);
    await copyClientIDListToOrder();
